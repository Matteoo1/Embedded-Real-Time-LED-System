...
attachInterrupt(digitalPinToInterrupt(button), ISR_, RISING); //Fallini the action will be when the button is pressed , RISING when button is relased.
...
void task_a()
{
  ...
  {
   xQueueReceive(firstQ, &pwm, portMAX_DELAY);     //take the last value from Q to use the number (pwm) to turn on the LED
...
void task_b()
{
  while(1)
  {
    xQueueReceive(secondQ, &pwm, portMAX_DELAY);     //take the last value from Q to use the number (pwm) to turn on the LED
